...
<div class="bg-white rounded-lg shadow-md p-6">
    <form action="{{ route('staf-tu.documents.update', $document) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <select id="ptk_id" name="ptk_id" required
                    ...
                    @foreach($ptkList as $ptk)
                        <option value="{{ $ptk->id }}" {{ old('ptk_id', $document->ptk_id) == $ptk->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" id="nama_dokumen" name="nama_dokumen" value="{{ old('nama_dokumen', $document->nama_dokumen) }}" required
            ...
            <div>
                ...
                <input type="date" id="tanggal_dokumen" name="tanggal_dokumen" value="{{ old('tanggal_dokumen', $document->tanggal_dokumen->format('Y-m-d')) }}" required
        ...
        <div class="mb-6">
            ...
            <textarea id="deskripsi" name="deskripsi" rows="3"
                      ...
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $document->deskripsi) }}</textarea>
            @error('deskripsi')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
            ...
            <div class="flex items-center justify-center w-full">
                <label for="file" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p class="mb-2 text-sm text-gray-500"><span class="font-semibold">Klik untuk upload file baru</span></p>
                        <p class="text-xs text-gray-500">PDF, JPG, JPEG (MAX. 10MB)</p>
                    </div>
                    <input id="file" name="file" type="file" class="hidden" accept=".pdf,.jpg,.jpeg" />
                </label>
            </div>
...
document.getElementById('file').addEventListener('change', function(e) {
    ...
    if (fileName) {
        ...
        document.getElementById('file-name').textContent = `File baru dipilih: ${fileName} (${displaySize})`;
    }
});
</script>
@endpush
